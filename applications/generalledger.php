...
class general_ledger_app extends application
{
	function general_ledger_app()
	{
		...
		$this->add_module(_("Dashboard"), "fa fa-home", "gl/dashboard");
...
// 		$this->add_lapp_function(0, _("&Deposits"), "gl/gl_bank.php?NewDeposit=Yes", 'SA_DEPOSIT', MENU_TRANSACTION);
		$this->add_lapp_function(1, _("&Payments"), "gl/inquiry/journal_inquiry.php?filtertype=1", 'SA_PAYMENT', MENU_TRANSACTION,'bank');
		$this->add_lapp_function(1, _("&Deposits"), "gl/inquiry/journal_inquiry.php?filtertype=2", 'SA_DEPOSIT', MENU_TRANSACTION,'money');


		$this->add_lapp_function(1, _("Bank Account &Transfers"), "gl/bank_transfer.php?", 'SA_BANKTRANSFER', MENU_TRANSACTION,'exchange');
...
// 			"gl/accruals.php?", 'SA_ACCRUALS', MENU_TRANSACTION);
		...
		if( defined('COUNTRY') && COUNTRY==65 ){
		    $this->add_lapp_function(2, _("GST Form 5"),"gst/form-5", 'SA_GLANALYTIC', MENU_INQUIRY,'list-ul');
		} else {
		    $this->add_lapp_function(2, _("GST Form 3"),"index.php/gst/form-3", 'SA_GLANALYTIC', MENU_INQUIRY,'list-ul');
		}
...
// 		    "index.php/bank/inquiry/statement?check=1", 'SA_BANKTRANSVIEW', MENU_INQUIRY,'list-ul');
		...
		$this->add_lapp_function(2, _("Ta&x"),
...
// 		    "index.php/bank/inquiry/check", 'SA_SALESALLOC', MENU_INQUIRY,'list-ul');
		...
		$this->add_rapp_function(3, _("Audit Trail"), "reporting/reports_main.php?Class=6&REP_ID=710", 'SA_GLREP', MENU_REPORT,'file-text');

// 		$this->add_rapp_function(2, _("GST Grouping Details"), "taxes/check.php", 'SA_GLREP', MENU_REPORT);
		$this->add_rapp_function(3, _("GST Grouping Details"), "index.php/tax/grouping-details", 'SA_GLREP', MENU_REPORT,'file-text');

		$this->add_module(_("Document Printing"), 'fa fa-print');
		$this->add_lapp_function(4, _("Bank Payment Voucher"), "reporting/reports_bank.php?type=".ST_BANKPAYMENT, 'SA_BANKACCOUNT', MENU_MAINTENANCE,'file-pdf-o');
		$this->add_lapp_function(4, _("Bank Deposit Voucher"), "reporting/reports_bank.php?type=".ST_BANKDEPOSIT, 'SA_BANKACCOUNT', MENU_MAINTENANCE,'file-pdf-o');
		$this->add_lapp_function(4, _("Bank Account Transfer Voucher"), "reporting/reports_bank.php?type=".ST_BANKTRANSFER, 'SA_BANKACCOUNT', MENU_MAINTENANCE,'file-pdf-o');
// 		$this->add_lapp_function(3, _("GL Journal Vouchers"), "reporting/reports_bank.php?type=".ST_JOURNAL, 'SA_BANKACCOUNT', MENU_MAINTENANCE,'file-pdf-o');
		$this->add_lapp_function(4, _("Bank Reconcile"), "index.php/bank/report/reconcile", 'SA_BANKACCOUNT', MENU_MAINTENANCE,'file-pdf-o');
