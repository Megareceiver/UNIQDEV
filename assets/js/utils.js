...
JsHttpRequest.request= function(trigger, form, tout) {
	...
	ATBefor_jsHttpRequest.inc();
	if( typeof trigger != 'string' && (trigger.getAttribute("ajax") && trigger.getAttribute("ajax") =='false') ) {
...
//				if(!objElement) alert('No element "'+id+'"');
        ...
        // Write errors to the debug div.
		 ...
		 jQuery(".page-content .container > div.alert ").remove();
		 $( errors ).insertBefore( ".page-content-inner" );
		 set_mark();
...
function price_format(post, num, dec, label, color) {
	...
	if( typeof user.ds == undefined || user.ds == undefined ){
		user.ds = '.';
	}
	...
	if( element.get(0).tagName =='INPUT' ){
		console.log('format number ='+ num + ' set value for input name = '+element.attr('name'));
		element.val(num);
	} else {
		element.html(num);
	}
