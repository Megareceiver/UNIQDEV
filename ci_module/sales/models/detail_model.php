...
//             return $query->row();
    ...
    function get_tran_details($tran_type,$tran_no, $return_query=false)
    {
        ...
        $this->db->from("debtor_trans_details AS d");
        if( $tran_type==ST_CUSTDELIVERY ){
            $this->db->select("d.stock_id, d.description As StockDescription, d.quantity, s.units, d.unit_price, d.discount_percent");
        } else {
            $this->db->select("d.*, d.description As StockDescription");
        }
