...
class Sales_Sale_Invoice_Model extends CI_Model
{
    ...
    function write_sales_invoice($invoice, $repost = false)
    {
        ...
        $cus_trans = array(
            ...
            'order_' => strlen($sales_order) > 0 ?$sales_order : 0,
            'ov_amount' => 0,
        ...
        foreach ($invoice->line_items as $line_no => $invoice_line) {
            ...
            $cus_trans_detail = array(
                ...
                'quantity' => intval(strtonumber($invoice_line->qty_dispatched) ),
                'unit_price' => floatval( strtonumber($invoice_line->line_price()) ),
                'unit_tax' => 0,
                'discount_percent' => floatval( strtonumber($invoice_line->discount_percent) ),
                'standard_cost' => floatval( strtonumber($invoice_line->standard_cost) ),
            ...
            $is_delivery_item = array_key_exists($line_no,$invoice->item_delivery_to_invoice);
            if( $is_delivery_item && $invoice->trans_type == ST_SALESINVOICE ){
                $delivery_item = $invoice->item_delivery_to_invoice[$line_no];
                $this->cus_trans->update_delivery_item($delivery_item->id,$invoice_line->qty_dispatched);
            }
