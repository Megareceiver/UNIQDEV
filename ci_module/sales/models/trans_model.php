...
class Sales_Trans_Model extends CI_Model {
    ...
    function write_customer_trans_detail($trans_type, $trans_no=0,$data,$check_existed=true){
        ...
        db_query($sql, "The debtor transaction detail could not be written line 96");
        return true;
    }

    function update_delivery_item($id=0,$qty_done=0){

//        $this->db->reset();
//        $where = array("id"=>$id,'debtor_trans_type'=>ST_CUSTDELIVERY);
        $qty_done = intval($qty_done);
//        $sql = $this->db->update('debtor_trans_details',array('qty_done'=>"qty_done-$qty_done"),$where,NULL,true );
        $sql = "UPDATE `debtor_trans_details` SET `qty_done` = `qty_done`+$qty_done WHERE `id` =  $id";

        db_query($sql, "The debtor transaction detail could not be update 105 ");
