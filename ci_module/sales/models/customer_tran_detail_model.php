...
class SalesCustomerTranDetail_Model extends CI_Model
{
    ...
    function get_items_delivery_not_yet_invoice(){
        $this->db->from("debtor_trans_details AS de")->select("de.*");

        $this->db->join("debtor_trans as tran","tran.trans_no=de.debtor_trans_no AND tran.type=de.debtor_trans_type","LEFT")
                ->select("tran.order_ AS order_no")
        ;
        $this->db->where("de.debtor_trans_type",ST_CUSTDELIVERY);

        $this->db->where("de.qty_done < de.quantity");
