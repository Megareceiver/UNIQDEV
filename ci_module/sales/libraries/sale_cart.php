...
class SalesSaleCartLib
{
    var $mode = 1;
    var $cart = NULL;
    var $tran_detail_model = NULL;
    function __construct() {
        if( empty($this->cart) AND isset($_SESSION['Items']) ){
            $this->cart = $_SESSION['Items'];
        }
        $this->tran_detail_model = module_model_load('customer_tran_detail','sales');
        $this->cart_actions();
    }
...
//         end_row();
    ...
    function overdue_items_box($editable_items = false){
        div_start('items_table',$trigger = null, $non_ajax = false, $attributes = ' class="pb-4" ');
        start_table(TABLESTYLE);
        $th = array(
            _("Delivery"),
            ...
            $this->cart->tax_included ? _("Price after Tax") : _("Price before Tax"),
            _("Tax"),
            _("Total"),
            ""
        );
...
//        $low_stock = $this->cart->check_qoh($_POST['OrderDate'], $_POST['Location']);

        if( empty($this->cart->item_delivery_to_invoice) ){
            $deliveriedItems = $this->tran_detail_model->get_items_delivery_not_yet_invoice();
        } else {
            $deliveriedItems = $this->cart->item_delivery_to_invoice;
        }

        if( count($deliveriedItems) > 0 ){
            foreach ($deliveriedItems AS $k=>$ite){
                $invoiced = array_key_exists($k,$this->cart->line_items);
                if( $invoiced ){
                    $invoice_line = $this->cart->line_items[$k];
                    //if( $invoice_line[""] )

                }
                start_row();
                //label_cell( anchor(sprintf("sales/detail/delivery-note/%d",$ite->debtor_trans_no),$ite->debtor_trans_no));
                label_cell( get_customer_trans_view_str($ite->debtor_trans_type, $ite->debtor_trans_no));

                $order_view = "";
                if( isset($ite->order_no) ){
                    $order_view = get_customer_trans_view_str(ST_SALESORDER, $ite->order_no);
                }
                label_cell( $order_view);

                label_cell($ite->stock_id);
                label_cell($ite->description);
                label_cell($ite->quantity);
                label_cell($ite->qty_done);
                if( $invoiced ){
                    amount_cell($invoice_line->quantity);
                } else {
                    input_number_cells("quantity[$k]",$ite->quantity-intval($ite->qty_done));
                }

                if( $invoiced ){
                    amount_cell($invoice_line->price);
                } else {
                    input_money_cells("price[$k]", $ite->unit_price);
                }

                tax_cell($ite->tax_type_id);
                amount_cell($ite->unit_price*$ite->quantity);
                if( $invoiced ){
                    tbl_edit("EditItemDeliveryToInvoice[$k]",$k);
                } else {
                    tbl_add("AddItemDeliveryToInvoice[$k]",$k);
                }

                end_row();
                $this->cart->item_delivery_to_invoice[$k] = $ite;
            }
        }

        $display_total = 0;
        label_row(_("Total"), $display_total, "colspan=9 align=right", "align=right", 3);
...
//             icon_submit_cells('ClearFields', _("Reset"), 'warning', 'fa-refresh', true ,_("Clear all GL entry fields"));
    ...
    private function cart_actions(){
        ...
        if (isset($_POST['AddItemDeliveryToInvoice'])) {
            $line_submit = input_post("AddItemDeliveryToInvoice");
            if( is_array($line_submit) ){
                $line_submit = reset($line_submit);
            }
            add_item_delivery_to_invoice_cart($line_submit);
        }

        if (isset($_POST['EditItemDeliveryToInvoice'])) {
            $line_submit = input_post("EditItemDeliveryToInvoice");
            if( is_array($line_submit) ){
                $line_submit = reset($line_submit);
            }
            unset($this->cart->line_items[$line_submit]);

            page_modified();

            line_start_focus_sale_cart();
            //add_item_delivery_to_invoice_cart($_POST['AddItemDeliveryToInvoice']);
        }
