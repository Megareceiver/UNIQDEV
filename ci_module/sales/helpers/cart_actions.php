...
function add_item_delivery_to_invoice_cart ($line_id=0)
{
    $cart = $_SESSION['Items'];
    if( is_object($cart) && get_class($cart)=="cart" && isset($cart->item_delivery_to_invoice) ){
        if( !array_key_exists($line_id,$cart->item_delivery_to_invoice) ){
            return false;
        }
        $item = $cart->item_delivery_to_invoice[$line_id];
        $item_qty = input_post("quantity");
        if( is_array($item_qty) ){
            $item_qty = $item_qty[$line_id];
        }
        $item_price = input_post("price");
        if( is_array($item_price) ){
            $item_price = $item_price[$line_id];
        }

        if( $item_qty > $item->quantity ){

            display_error( _( "The quantity can not great than quantity delivery."));
            set_focus("quantity[$line_id]");
            return false;
        }

        $cart->add_to_cart_new($item->tax_type_id, $line_id, $item->stock_id, $item_qty, $item_price, $item->discount_percent, $item->qty_done, 0, $item->description);

        unset($_POST['quantity'], $_POST['price'],$_POST["AddItemDeliveryToInvoice"]);
        page_modified();
