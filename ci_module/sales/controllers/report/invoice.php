...
//         $ci = get_instance();
    ...
    private function pdf_print($tran=NULL){
        ...
        if( strlen($tran->address) > 0 ){
            $this->rep->params['delivery_info']['Sold To'] .= "\n".$tran->address;
        ...
        $gl_items = $this->customer_trans_model->trans_detail_gl('*',array('debtor_trans_type'=>ST_SALESINVOICE,'debtor_trans_no'=>$tran->trans_no));
//        if( empty($items) ){
//            return;
//        } else {
            $this->rep->NewPage();
            ...
            if( !empty($items) || !empty($gl_items) ){
                ...
                foreach ($gl_items AS $detail){
                    ...
                    $this->rep->TextCol(1, 2,   $detail->gl_code, -2);
                    $this->rep->TextCol(2, 3,   wordwrap($detail->description,40,"\n",true), 0);
                    $this->rep->TextCol(3, 4,	number_format2($sign*$detail->quantity,get_qty_dec($detail->stock_id)), -21);
                    $this->rep->TextCol(4, 5,	$detail->units, -2);
                    $this->rep->TextCol(5, 6,	number_total($detail->unit_price), -2);
                ...
                $discountTotal += $tran->ov_discount;
                $SubTotal += $tran->ov_amount;
                $taxTotal += $tran->ov_gst;
            }
