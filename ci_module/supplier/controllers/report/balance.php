...
//             $rate = $convert ? get_exchange_rate_from_home_currency($myrow['curr_code'], Today()) : 1;
            ...
            $line_total = array('debit'=>abs($bal->debit),'credit'=>abs($bal->credit),'balance'=>0);


            //$line_total['balance'] += floatval($bal->debit) - floatval($bal->credit);
            $line_total['balance'] += $line_total["debit"] - $line_total["credit"];

            if( $line_total['balance'] < 0 ){
//                $rep->TextNum(5, 6, ($bal->debit));
//                $rep->TextNum(6, 7, ($bal->credit));

                $rep->TextNum(6, 7, abs($line_total['balance']));
                $rep->TextNum(8, 9, abs($line_total['balance']));
            } else {
                $rep->TextNum(5, 6, $line_total['balance']);
