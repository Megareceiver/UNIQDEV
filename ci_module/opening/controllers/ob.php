...
//            'homecurr'=>false
    ...
    function form($id = 0)
    {
        ...
        if ($id) {
            ...
            $data["currency"]['value'] = curr_default();
        }
        ...
        if ($edit) {
            $atype = true;
//            $atype = false;
            submit('submit', _("Save"), true, '', $atype, 'save');
        }
        box_footer_end();
    }
    ...
    function customer_submit(){
        global $Ajax;
        $data = array();
        $rate = input_post("curr_rate");
        if( strlen($rate) < 1 || abs(floatval($rate)) < 0 ){

            display_error(_("Please input Currency and Exchange Rate"));
            return false;
