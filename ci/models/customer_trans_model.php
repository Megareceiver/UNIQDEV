...
// 		$this->trans->where(array('de.unit_price >'=>0,'de.quantity <>'=>0));
        ...
        $this->trans->where("pro.stock_id IS NOT NULL");
    ...
    function trans_detail_gl($select='*',$where=null,$trans_type=ST_SALESINVOICE){
        if( !$select ){ $select = '*'; }

        $select.=',tax.rate AS tax_rate, de.id AS item_id, "" AS units, 1 AS qty, de.unit_price AS price, de.tax_type_id';
        $select.=", IF ( de.description <> '', de.description, CONCAT(gl.account_name, ' ', gl.account_code)  ) AS description";
        $select.=", IF ( de.description <> '', '', gl.account_name) AS long_description";
...
// 		$this->trans->where(array('de.unit_price >'=>0,'de.quantity <>'=>0));
        ...
        $this->trans->where("gl.account_code IS NOT NULL ");
        $this->trans->join('chart_master AS gl', 'gl.account_code= de.gl_code', 'left');
...
// 			$sql = $this->trans->update('debtor_trans_details',$data, array('debtor_trans_no'=>$trans_no),1,true );
		...
		db_query($sql, "The debtor transaction detail could not be written 544");
		return true;
	}
